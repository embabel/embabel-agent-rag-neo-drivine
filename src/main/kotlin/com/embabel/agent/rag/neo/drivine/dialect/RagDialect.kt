 ...
 * FalkorDB, and Neptune Analytics for vector indexing, search, fulltext
 * indexing, constraint creation, and embedding storage.
 *
...
interface RagDialect {
     ...
     * Neptune Analytics skips — vector indexes are defined at graph creation time.
     */
     ...
     *
     * Neptune Analytics skips — no native fulltext search.
     */
     ...
     * Neptune Analytics has no property-level constraints.
     */
     ...
     * For Neptune Analytics it is written via `neptune.algo.vectors.upsert()`.
     *
    ...
    companion object {
        ...
        fun forDatabaseType(type: DatabaseType): RagDialect = when (type) {
            ...
            DatabaseType.NEPTUNE -> NeptuneAnalyticsRagDialect()
            DatabaseType.MEMGRAPH -> MemgraphRagDialect()
            else -> throw IllegalArgumentException(
                "No RAG dialect for database type: $type. Supported types: NEO4J, FALKORDB, NEPTUNE, MEMGRAPH"
            )
        }
        ...
        fun forName(name: String): RagDialect = when (name.uppercase()) {
            ...
            "NEPTUNE", "NEPTUNE_ANALYTICS" -> NeptuneAnalyticsRagDialect()
            "MEMGRAPH" -> MemgraphRagDialect()
            else -> throw IllegalArgumentException(
                "No RAG dialect named: $name. Supported: NEO4J, FALKORDB, NEPTUNE, MEMGRAPH"
            )
        }
    }
}
